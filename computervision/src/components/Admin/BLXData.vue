<template>
  <v-alert style="font-size: 25px; font-weight: 600">Giấy phép lái xe</v-alert>
  <v-table>
    <thead>
      <tr>
  ...
  methods: {
    async getData() {
      try {
        ...
        this.desserts = await DemoAPI.getBLX(this.page, 10);
        this.pageCount = (await DemoAPI.getBLXCount()) / 10 + 1;
