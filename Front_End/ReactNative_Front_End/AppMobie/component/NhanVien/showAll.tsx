...
const ShowAll = () => {
  const tableHead = ['Họ', 'Tên', 'Giới tính', 'Ngày sinh', 'Xóa'];
  ...
  const handleButtonClick = async (maKhoHang: Number) => {
    await remove(maKhoHang);
    await fetchData();
  };
        ...
        <Table borderStyle={{borderWidth: 1}} style={styles.table}>
          ...
          <Rows
            data={tableData.map(row => [
              ...
              row.ngaySinh == null
                ? null
                : row.ngaySinh.toString().substring(0, 10),
              <View>
                <TouchableOpacity
                  onPress={() => handleButtonClick(row.maNhanVien)}>
